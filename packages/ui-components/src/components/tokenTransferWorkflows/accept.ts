...
import { assetAndTokenConfigGetItemByTokenTypeName } from '@local/f-model/dist/config';
...
type ProposeSubmitPropsAdapterComponents<T extends ProposeSubmitTSup> = {
    [C in keyof ProposeSubmitPropsAdapter<T>]: (props: ProposeSubmitPropsAdapter<T>[C]) => ReactElement;
};

// ---

const adapter: ProposeSubmitPropsAdapterComponents<ITs['tokenTransferAccept']> = {
    proposalActionDialog: (props): ReactElement => {
        ...
        return ModalPopup1Factory({
            ...
            width: 376,
            elements: [
                { type: 'headerText', text: 'Accept this transfer' },
                { type: 'spacer', size: 8 },
    ...
    responseReceived: (props) => {
        ...
        if (props.response.isOk) {
            ...
            let tokenTypeLabel: string =
                resultC(assetAndTokenConfigGetItemByTokenTypeName(resultC(props.params.assetAndTokenConfig).unwrapOr([]), order.tokenTypeName))
                    .map(item => item.tokenTypeLabel)
                    .unwrapOrElse(e => `[${e}]`);
            ...
            return ModalPopup1Factory({
                ...
                width: 376,
                elements: [
                    { type: 'headerText', text: 'You accepted this transfer' },
                    { type: 'spacer', size: 8 },
                    // @todo: To get unit of token
                    // @todo: To format account ID
                    { type: 'text', lineHeight:1.5, text: `${order.amount} ${tokenTypeLabel} will be deposited to your account [Account ID: ${order.toAccountID}].` },
                    { type: 'spacer', size: 24 },
                    {
                        type: 'actionBar',
                        elements: [
            ...
            return ModalPopup1Factory({
                ...
                width: 280,
                elements: [
                    { type: 'headerText', text: 'Sorry, an error occurred while accepting the transfer',color:'red' },
                    { type: 'spacer', size: 8 },
...
export type AcceptProps = Props<ITs['tokenTransferAccept']>;
export const Accept = (props: AcceptProps): ReactElement => {

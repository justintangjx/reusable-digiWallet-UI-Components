...
export const assetDeregistrarAppointmentAcceptWorkflowS1C = (props: Props) => {
    ...
    const proposal = props.state !== 'proposed' ?
        ...
        s1.modalPopup(true, {width: 367, height: 'content', padding: 20 },
            s1.linearLayoutV({width: 'content', height: 'content', alignH: 'left', alignV: 'top'  }, [
                s1.text('Accept this appointment',{fontWeight: 500, fontSize: 20,} ),
                s1.linearLayoutSpacer ({size:8}),
                s1.text('By accepting this appointment, you hereby accept responsibility and agree to abide by the redemption process.', {fontSize:16}),
                s1.linearLayoutSpacer ({size:8}),
                s1.text('You also acknowledged that there are enough asset for redemption in your custody.', {fontSize:16,fontWeight:500}),
                s1.linearLayoutSpacer ({size:8}),
                s1.text('This action cannot be undone. If you are sure, click “CONFIRM”.', {fontSize:16}),
    ...
    const responseReceived = props.state !== 'responseSuccess' ?
        ...
        s1.modalPopup(true, {width: 280, height: 'content', padding: 20 },
            s1.linearLayoutV({width: 'content', height: 'content', alignH: 'left', alignV: 'top' }, [
                s1.text('You accepted the appointment', {fontWeight: 500, fontSize: 20}),
                s1.linearLayoutSpacer({size:8}),
                s1.text('You are required to deregister asset upon redemption. Owner will be notified.', {fontSize:16,}),
                s1.linearLayoutSpacer({size:24}),
                s1.linearLayoutH({alignH:'right', width:'full'}, [
